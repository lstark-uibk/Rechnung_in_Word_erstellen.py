...
archive_which_invoices_path = f"C:\\Users\\rosma\\Documents\\Rechnungen\\{currentyear}\\Rechnungen{currentyear}.xlsx"
savefiles = False
...
if result == answer1:
    ...
    thisinvoicenumber = f"{currentyear}-{lastinvoicenumber + 1:03}"
...
if result == answer2:
    ...
    thisinvoicenumber = tk.simpledialog.askstring('Rechnungsnummer',
                                               "Dann kannst du sie jetzt selber eingeben (in dem Format 2023-012):",
                                               parent=root)
    root.destroy()
    ...
    while not re.match(invoicenumber_pattern,thisinvoicenumber):
        ...
        # shows a dialogue with a string input field
        thisinvoicenumber = tk.simpledialog.askstring('Rechnungsnummer',
                                                   "Die letze eingetragen Rechnungsnummer hatte nicht das richtige Format. \nGib sie in dem Format ein wie 2024-001 wobei die erste Nummer mit dem aktuellen Jahr ersetzt wird und die zweite mit der Rechnungsnummer:",
                                                   parent=root)
        root.destroy()
...
namehourdata = namehourdata[(namehourdata["Datum"].dt.date>=invoice_start_date)&(namehourdata["Datum"].dt.date<=invoice_end_date)]   #delete everything brfore lastinvoicegroup
...
clientdata["Heute"] = today.strftime("%d.%m.%Y")
...
outputfile_path = os.path.join(base_dir / outputdir_path, "RE " + thisinvoicenumber + " " + name + " "+ today.strftime("%d.%m.%Y") + ".docx")
...
if savefiles:
    ...
    with open(invoicenumber_path, 'a') as f:
        if lastinvoiceyear == int(today.strftime("%Y")):
            f.write(thisinvoicenumber + "\n" )
        else:
            f.write(today.strftime("%Y") + "-" + str(1) + "\n")
